...
- A missing or empty index is normal for a new project. `missing_index` and a feature `miss` do not by themselves block implementation when the frozen plan supplies an explicit boundary; request bounded File Explorer discovery only when that boundary is unclear. `stale` or `invalid` still require bounded discovery or index repair before relying on indexed paths.
