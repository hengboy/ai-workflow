...
- Coding must create one project-local temporary worktree under `<project>/.worktrees/<name>` before implementation; all implementation, validation and per-step commits happen inside that single worktree, with `.worktrees/` in `.gitignore`.
