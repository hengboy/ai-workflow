...
- `src/context/discovery`: deterministic bounded repository scanning, optional user-owned `.ai-workflow/project.yml`, TypeScript/JavaScript and Java structural adapters, and canonical navigation building.
...
- Navigation JSON is authoritative and `navigation.md` is rendered from it. Structural `file` roots are validated by file existence and coverage; semantic `exported-symbol` roots by symbols and relations; unsupported semantic languages are rejected.
