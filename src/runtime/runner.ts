...
export async function startRun(options: StartOptions): Promise<RunRecord> {
  ...
  if (workflow.host !== options.host) throw new Error(`Host mismatch: workflow=${workflow.host}, requested=${options.host}`); const validation = await validateWorkflow(workflow); if (!validation.valid) throw new Error(validation.errors.join('; ')); const project = resolve(options.project ?? process.cwd()); await verifyApproval(workflowPath, workflow, project);
  ...
  const now = new Date().toISOString(); const runId = `${workflow.plan_id}-${Date.now().toString(36)}-${randomUUID().slice(0, 8)}`;
