...
async function executeGitNode(record: RunRecord, workflow: Workflow, node: Workflow['nodes'][number]): Promise<unknown> { const { plan_worktree: planPath, plan_branch: planBranch, start_head: startHead } = record.resources; if (!planPath || !planBranch || !startHead) throw new Error('Incomplete plan worktree resources'); const plan: Worktree = { path: planPath, branch: planBranch, base: startHead }; const taskId = node.task_id; if (node.id.endsWith('-setup') && taskId) { const task = await createTaskWorktree(record.project, plan, record.run_id, taskId); record.resources.task_worktrees[taskId] = task.path; record.resources.task_branches[taskId] = task.branch; return task; } if (node.id.endsWith('-commit') && taskId) { const path = record.resources.task_worktrees[taskId]; if (!path) throw new Error(`Missing task worktree: ${taskId}`); const implementation = workflow.nodes.find((item) => item.task_id === taskId && item.id.endsWith('-implement')); const commit = await commitTask(path, taskId, implementation?.write_scope ?? []); record.resources.commits[taskId] = commit; await mergeTask(plan.path, commit); await removeOwnedWorktrees(record.project, [path]); delete record.resources.task_worktrees[taskId]; return { commit }; } if (node.id === 'plan-integrate') { const commit = await integratePlan(record.project, plan.branch, record.resources.start_branch, record.resources.start_head); record.resources.merge_commit = commit; await removeOwnedWorktrees(record.project, [plan.path]); delete record.resources.plan_worktree; await deleteOwnedBranches(record.project, [...Object.values(record.resources.task_branches), plan.branch]); return { commit }; } throw new Error(`Unknown Git lifecycle node: ${node.id}`); }
