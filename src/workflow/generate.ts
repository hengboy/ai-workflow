...
import { normalizeProjectPaths, taskReadScopeDiagnostics, type TaskReadAuthorization } from './read-scope.js';
...
export async function generateWorkflow(planDirectory: string, host: Host, concurrency = 3): Promise<Workflow> {
  ...
  for (const task of parsedTasks) {
    ...
    const authorization: TaskReadAuthorization = { task_id: task.id, exact_paths: locator.paths, module_directories: [] };
  ...
  const tasks = serializeConflicts(parsedTasks.length ? parsedTasks : [{ id: 'task-001-plan', requirements: plan.requirements, acceptanceCriteria: plan.acceptanceCriteria, dependsOn: [], surface: 'cross-stack', locatorReadOrder: [], readScope: fixedTaskContext, writeScope: [], testCommands: [], path: `${planScope}/plan.md` }]);
  ...
  const validation = await validateWorkflow(workflow); if (!validation.valid) throw new Error(`Generated workflow is invalid: ${validation.errors.join('; ')}`); return workflow;
}
