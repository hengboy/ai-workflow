...
workflow.command('generate').description('Generate canonical .ai-workflow/plans/<plan-id>/workflow.json').requiredOption('--plan <directory>').requiredOption('--host <host>').option('--adjustments-stdin').action(async ({ plan, host, adjustmentsStdin }: { plan: string; host: Host; adjustmentsStdin?: boolean }) => {
  const directory = resolve(plan); const project = resolveProjectRoot(resolve(directory, '../../..')); const document = await readPlan(directory); const canonicalDirectory = join(project, '.ai-workflow', 'plans', document.planId); if (directory !== canonicalDirectory) throw new Error(`Workflow plan directory must be canonical: ${canonicalDirectory}`); let value = await generateWorkflow(directory, host); if (adjustmentsStdin) { const text = await new Promise<string>((done) => { let data = ''; process.stdin.setEncoding('utf8'); process.stdin.on('data', (chunk: string) => { data += chunk; }); process.stdin.on('end', () => done(data)); }); const adjustment = JSON.parse(text) as { operations: Array<Record<string, unknown>> }; const validator = await schemaValidator('adjustment.schema.json'); if (!validator(adjustment)) throw new Error(formatSchemaErrors(validator.errors)); value = applyAdjustments(value, adjustment.operations); const validation = await validateWorkflow(value, project); if (!validation.valid) throw new Error(validation.errors.join('; ')); }
  const target = join(directory, 'workflow.json'); await writeJson(target, value); print({ workflow: target, explanation: explainWorkflow(value) });
});
