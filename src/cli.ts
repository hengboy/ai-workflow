...
import { generateAdrIndex, verifyAdrIndex } from './adr/index.js';
...
const adr = program.command('adr'); adr.command('index').option('--project <project>', projectOption, process.cwd()).option('--verify').action(async ({ project, verify }: { project: string; verify?: boolean }) => { if (verify) { const result = await verifyAdrIndex(project); print(result); if (!result.consistent) process.exitCode = 1; } else { print(await generateAdrIndex(project)); } });
