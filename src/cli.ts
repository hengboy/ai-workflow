...
workflow.command('generate').requiredOption('--plan <directory>').requiredOption('--host <host>').option('--output <path>').option('--adjustments-stdin').action(async ({ plan, host, output, adjustmentsStdin }: { plan: string; host: Host; output?: string; adjustmentsStdin?: boolean }) => {
  let value = await generateWorkflow(resolve(plan), host); if (adjustmentsStdin) { const text = await new Promise<string>((done) => { let data = ''; process.stdin.setEncoding('utf8'); process.stdin.on('data', (chunk: string) => { data += chunk; }); process.stdin.on('end', () => done(data)); }); const adjustment = JSON.parse(text) as { operations: Array<Record<string, unknown>> }; const validator = await schemaValidator('adjustment.schema.json'); if (!validator(adjustment)) throw new Error(formatSchemaErrors(validator.errors)); value = applyAdjustments(value, adjustment.operations); const validation = await validateWorkflow(value, resolveProjectRoot(resolve(plan, '../../..'))); if (!validation.valid) throw new Error(validation.errors.join('; ')); }
