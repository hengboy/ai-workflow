...
describe('manage ai-workflow skill', () => {
  it('initializes new projects and safely updates managed projects through the CLI', async () => {
    const root = packagePath('templates', 'skills', 'manage-ai-workflow');
    ...
    expect(text).toMatch(/^name: manage-ai-workflow$/m);
    ...
    expect(metadata.interface?.display_name).toBe('Manage AI Workflow');
    expect(metadata.interface?.short_description).toBe('Initialize or safely update an ai-workflow project.');
    expect(metadata.interface?.default_prompt).toContain('$manage-ai-workflow');
  });
});
