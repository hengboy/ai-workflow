...
- Plan-to-tasks validates the frozen pair, previews the complete graph and its parallel phases with the critical path, obtains approval, and creates immutable `tasks/<taskId>.md` files plus `tasks/execution-order.yaml`, the frozen schedule: an ordered list of non-empty `parallel` phases covering every task exactly once, with every dependency in a strictly earlier phase and no shared normalized write-scope path inside one phase. The schedule is machine-readable, has no `.zh.md` or `.i18n.yaml` sibling, and matches the approved preview. It never edits frozen plans.
