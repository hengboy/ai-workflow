...
Return a Markdown report using the following level-two headings, including `## Found Paths` for discovery results. Do not return a JSON envelope. Never fabricate a path.

### Status
Return only `done`, `blocked`, or `failed`.
### Summary
Return the discovery outcome and unresolved questions for the assigned objective.
### Found Paths
Return exact discovered paths as a Markdown list, not a JSON array or a changed-path list; this role is read-only.
### Evidence
Return symbols, line evidence, call-chain direction and relevant test paths. Report supplied check commands and exit codes when available; mark unexecuted checks as `skipped` with reasons.
### Support Requests
Return actionable requests for authorized roots or missing evidence, or state none.
