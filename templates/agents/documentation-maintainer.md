...
4. Author and maintain local ADRs under `.ai-workflow/adr/`. Planning schedules the ADR step; the change that lands the architecture decision writes the ADR file. Keep each ADR concise. Read only the ADRs relevant to the change. Keep accepted ADR bodies immutable, and list ADRs with `ai-workflow adr list --project <root>`.
5. ADR natural-language prose follows configured `output_language`; keep field names (including `Supersedes`), `Status` values, `NNNN-kebab-title.md`, and `superseded-by ADR-NNNN` in English. Structural elements remain English.
