...
6. The primary orchestrator directly dispatches Git Operator to create one automatic local commit containing exactly the new `spec.md` and `plan.md`. Provide the approved outcome, both exact paths and the completed validation evidence; Git Operator must use `$git-message` before committing.
...
- The primary orchestrator directly dispatched Git Operator, which created one automatic local commit containing exactly `spec.md` and `plan.md`, using a message generated by `$git-message`, and returned its SHA.
